...
def get_all_doc_owners_names(set_name):
    users_list = []
    for current_document in documents:
        try:
            ...
            pass
    set_users_list = set(users_list)
    if set_name & set_users_list:
...
def add_new_shelf(dict_name, shelf_number=''):
    if shelf_number not in dict_name.keys():
        dict_name[shelf_number] = []
        return dict_name
...
def delete_doc(list_name, user_doc_number=''):
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for current_document in list_name:
            doc_number = current_document['number']
            if doc_number == user_doc_number:
                list_name.remove(current_document)
